...
const Navbar: React.FC = () => {
  ...
  const navLinks = [
    ...
    { name: 'Equipo', href: '/equipo', hasDropdown: false },
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-lg py-2 shadow-md border-b border-gray-200/50' : 'bg-white/80 backdrop-blur-md py-3 shadow-sm border-b border-white/50'}`}>
          ...
          <div className="hidden md:flex items-center gap-12">
            {navLinks.map((link) => (
              <div key={link.name} className="relative group">
                <Link
                  to={link.href}
                  className={`font-outfit font-medium text-[16px] transition-all py-2 block flex items-center gap-1.5 hover:scale-105 ${isActive(link.href) ? 'text-porta font-bold' : 'text-gray-600 hover:text-porta'
                    }`}
                >
                  {link.name}
                  {link.hasDropdown && (
              ...
              <div key={link.name}>
                {link.hasDropdown ? (
                  ...
                  <Link
                    to={link.href}
                    onClick={() => setIsMenuOpen(false)}
                    className={`block px-4 py-3 rounded-2xl text-base font-bold transition-colors ${isActive(link.href) ? 'text-porta bg-porta-accent/30' : 'text-gray-700 hover:text-porta hover:bg-porta-accent/50'
                      }`}
                  >
                    {link.name}
                  </Link>
